...
for X, n in zip([X_, X_old, X_new], ['both', 'old',  'new']):
    ...
    for idx0, idx1 in [(0, 1), (1, 0), (0, 2), (2, 0), (1, 2), (2, 1)]:
        lab=f'{idx0}to{idx1}'
        data=(X[idx0], X[idx1])
        Res_all[lab] = pd.DataFrame(columns = ['gc', 'bic', 'Fval', 'pval','p_emp',  'F_null_p', 'F_null_m', 'F_null_sd','gc_s_p', 'gc_s_m', 'gc_s_sd', 'lag (tp)', 'lag (ms)', 'tmean', 'tstart', 'tend'])
        for e in range(len(time)-window_len):
            start = e
            tend = e+ window_len
            res = pd.DataFrame(columns = ['gc', 'bic', 'Fval', 'pval','p_emp', 'F_null_p', 'F_null_m', 'F_null_sd', 'gc_s_p', 'gc_s_m', 'gc_s_sd'])
            for l, mlag in enumerate(lags) :
                if ((data[0].shape[0] * ((window_len) - mlag)) / (2*mlag) >= 10) and (mlag <= abs(window_len)):
                    ...
                    res.loc[l, :] = [np.nan, np.nan,np.nan, np.nan , np.nan, np.nan,np.nan ,np.nan,np.nan , np.nan, np.nan]
        ...
        path_to_save = out_path + f'/{lab}_{n}_{method_perm}{n_perm}.csv'
